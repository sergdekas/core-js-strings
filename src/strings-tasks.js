...
function removeFirstOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    return str.substring(0, index).concat(str.substring(index + lenValue));
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  if (str.includes(value)) {
    ...
    return str
      ...
      .concat(str.substring(indexLast + lenValue));
  }
  return str;
}
...
function orderAlphabetically(str) {
  const subString = str.split('').sort().join('');
  return subString;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === ' ' || str[i] === ',' || str[i] === '?' || str[i] === '!') {
      ...
      str0 += str[i].toLowerCase();
      str1 = str[i].toLowerCase() + str1;
    }
  }
  return str0 === str1;
}
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < splitStr.length; i += 1) {
    if (splitStr[i].charCodeAt() < 65) {
      char = splitStr[i].charCodeAt(0);
      arr.push(String.fromCharCode(char));
    } else if (splitStr[i].toLowerCase().charCodeAt() < 110) {
      char = splitStr[i].charCodeAt(0) + 13;
      arr.push(String.fromCharCode(char));
    } else if (splitStr[i].toLowerCase().charCodeAt() > 76) {
      char = splitStr[i].charCodeAt(0) - 13;
      arr.push(String.fromCharCode(char));
    }
  }
